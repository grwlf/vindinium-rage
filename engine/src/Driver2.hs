...
{-
-- | Same as `driver_sim` but uses IORefs instead of State monad
driver_simIO
  :: FilePath
  -> (forall m g . (MonadRnd g m, MonadBot m, MonadIO m) => HashMap HeroId ((Game,HeroId) -> m ()))
  -> IO ()

driver_simIO file ctl = do

    {- Initial state query -}
    s0 <- loadState file

    stateRef <- newIORef $ DriverSim_State (s0^.stateGame) (HeroId 1) ctl
    genRef <- newIORef =<< initRndGen

    forever $ do

      gen <- readIORef genRef
      ds <- readIORef stateRef

      g   <- pure $ ds.>ds_g
      hid <- pure $ ds.>ds_hid
      bot <- pure $ ds.>ds_ctl.(idx hid)
      ...
      (r,gen') <- flip runRndT gen $ runBot $ bot (g,hid)

      case r of
        BotMove dir k -> do
          writeIORef stateRef $
            Lens.over ds_g (sim hid dir) $
            Lens.over ds_hid (nextHeroId) $
            Lens.over (ds_ctl.(idx hid)) (const k) $
            ds
